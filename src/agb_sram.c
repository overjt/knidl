...
/* SDK SRAM driver (libagbsrd, "SRAM_V112"). The core copy loops are copied
 * onto the stack and executed from there so no ROM access happens between
 * SRAM reads/writes. */
...
void ReadSram(const u8 *src, u8 *dest, u32 size)
{
    ...
    ((void (*)(const u8 *, u8 *, u32))((u32)readSramFast_Work + 1))(src, dest, size);
}
...
u32 VerifySram(const u8 *src, u8 *dest, u32 size)
{
    ...
    s = (const u16 *)((u32)VerifySram_Core);
    s = (const u16 *)((u32)s ^ 1);
    ...
    return ((u32 (*)(const u8 *, u8 *, u32))((u32)verifySramFast_Work + 1))(src, dest, size);
}
