...
| 6 | `0x080008E8-0x080072FF` | 0x6A18 | **Game code (Thumb): early subsystems** — decompilation in progress (issue #32). **The zone is NOT one compiler recipe** (lesson 3.18): `0x080008E8-0x08001B08` is `agbcc -O2`, the unit starting near `0x08001CC8` is `old_agbcc -O2`; read the leaf prologue (`push {lr}` present = agbcc) before assuming. Landed so far: `src/early_08e8.c` `0x080008E8-0x08000DE4` palette fade engine (`sub_080008e8` seeds the fade, `sub_08000934` runs it over the 512-colour buffer `0x03001270` into `0x03001A90` and publishes the active buffer in `0x03000FB0`); `src/early_10cc.c` `0x080010CC-0x080011AC` VBlank handler (sound vsync/main, OAM+palette DMA, copy queue, clears the frame flag `0x03001EC4`); `src/early_11ac.c` `0x080011AC-0x08001518` OAM/palette flush, key reader (`0x0300100C` held / `0x03000038` new / `0x03000B70` repeat), IO shadow flush, CpuSet copy-queue ring `0x03000B80-0x03000F7B`, HBlank/VCount handler setters; `src/early_1518.c` `0x08001518-0x08001B08` default (no-op) IRQ handler, VRAM transfer queue, sprite bucket lists; `src/early_1fd0.c` `0x08001FD0-0x08002378` (old_agbcc -O2) BG scroll / fade-blend resets, six fade-setup variants, boot header check against `0x0872EB2C`. Still asm: `sub_08000de4` `0x08000DE4` (frame-end tick: fade update, music volume ramp, VBlank spin, A+B+Start+Select soft reset, play-time clock `0x03000498[4]`) and `sub_08001cc8` `0x08001CC8` (affine-object emitter) — both structurally-correct C that is off only on register allocation; `sub_08001b08` `0x08001B08` is the OAM shadow builder, copied to IWRAM `0x03001F40` and run from RAM (verified position-independent: zero `bl`, pool inside the copied bytes). Remaining: `0x08002378-0x08007300`, incl. task entry `0x08005654` |
| 7 | `0x08007300-0x080CFA4B` | ~0xC8700 | **Game code + rodata (Thumb)** | ~2,650 Thumb BL targets, thumb-pointer tables throughout; interleaved rodata (pointer tables 122 runs ≥8 entries, e.g. 43-entry table @0x0803EC48). Tail (`0x080CF94C-0x080CFA4B`) is the m4a/mp2k XCMD handler block, named in #29 via the 12-entry jump table @`0x0860A3E8` (matches katam/pokeemerald `gXcmdTable` 1:1, `ply_xxx` at indices 0 AND 3) + per-handler `MusicPlayerTrack` field offsets: `ply_xxx` `0x080CF94C`, `ply_xwave` `0x080CF960`, `ply_xtype` `0x080CF9A8`, `ply_xatta` `0x080CF9BC`, `ply_xdeca` `0x080CF9D0`, `ply_xsust` `0x080CF9E4`, `ply_xrele` `0x080CF9F8`, `ply_xiecv` `0x080CFA0C`, `ply_xiecl` `0x080CFA18`, `ply_xleng` `0x080CFA24`, `ply_xswee` `0x080CFA38`. The old `0x080CFA40` segment boundary cut `ply_xswee` in half (its tail was seg 8's "unidentified SDK helper" `gUnk_080cfa40`, 0 BL callers because XCMD handlers are table-dispatched only); boundary moved to `0x080CFA4C` in #29. **The whole m4a/mp2k engine occupies the region tail `0x080CD89C-0x080CFA4B` and is fully named in the symbol DB (issue #31) — see §8; carved out of the `game_code_and_rodata` segment into dedicated segments `m4a_1` (asm core) + `m4a` (C driver) in #52** |
