...
| 8 | `0x080CFA40-0x080CFA7E` | 0x3F | **SDK syscall wrappers (Thumb)** | `svc N; bx lr` pairs: `0x080CFA50 svc 0x0A`, `0x080CFA54 svc 0x0C` (CpuFastSet), `0x080CFA58 svc 0x0B` (CpuSet), `0x080CFA5C svc 0x06` (Div), `0x080CFA68 svc 0x13`, `0x080CFA6C svc 0x12`, `0x080CFA70 svc 0x11`, `0x080CFA74 svc 0x25`, `0x080CFA7C svc 0x28`; `0x080CFA80` reset helper (`svc 1; svc 0`) |
| 9 | `0x080CFA9C-0x080CFDDB` | ~0x340 | **C library + SRAM driver (Thumb)** | byte/word copy loops @0x080CFA9C; SRAM driver accesses `0x0E0000xx` with `REG_WAITCNT (0x04000204)` waitstate setup @0x080CFAC0-0x080CFB94; interworking veneer `ldr ip,[pc]; bx ip` @0x080CFDDC → Thumb `0x08005654`; 14-entry IRQ handler table @0x080CFDE8 (13× default `0x08001519`, slot 2 = `0x080010CD`) |
