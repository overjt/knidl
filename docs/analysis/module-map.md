...
| M06 | `0x0801A8C8-0x08021B17` | 28.6 KiB | 56 | 0 | ***** | terrain / collision query (pure leaf) - **partial (#84)**, 21/55 (`sub_08021b0e` was a census false positive) |
...
* **Census** the 188-row census was wrong in 22 places (commit "Fix
  ...
  heuristic invented out of graphics-blob words, seven real entries the
  prologue filter rejected because `-fprologue-bugfix` leaves a pool-loading
  leaf without a `push`, and ten dead exports (copies of a host's tail
  dispatch that no 4-aligned ROM word references).  All 22 are curated in
  `tools/symdb.py`.
* **What it turned out to be** nine ROM task types built to the same
  three-table pattern as M22/M24/M25/M26:
  * eight **class-3** types (#27 `sub_0807fcbc`, #31 `sub_08080dd0`,
    #32 `sub_08080400`, #38 `sub_080819b4`, #39 `sub_08081f60`,
    #40 `sub_08082008`, #42 `sub_080826d8`, #43 `sub_08082de4`) whose bodies
    are one `sub_08002e98(Task.unk73, N, table)` dispatch each - tables
    `0x08741488`, `0x087414B4`, `0x08741544`, `0x087415B8`, `0x08741604`
    (shared by #39 and #40), `0x08741640` and `0x0874176C`;
  * one **class-4** type, #175 `sub_08080c48`, which is the coroutine itself:
    it re-seats its actor next to `gUnk_03002790[Task.unk44]` every cycle and
    hangs `sub_08080d58` off `Task.unk04`;
  * 21 **entry/hook script pairs** underneath them, each an entry that writes
    `Task.unk04` and dispatches `Task.unk14`, plus a hook that dispatches
    `Task.unk15` (`0x08741390`/`0x0874139C`, `0x087413A8`/`0x087413B4`,
    `0x087413C0`/`0x087413C8`, `0x087413D0`/`0x087413D4`,
    ...
    `0x0874167C`);
  * the `unk73` quartets at `0x08741BB8`, `0x08741BF4`, `0x08741C44` and
    `0x08741C64` are the `s32`-returning guards.
* **Shared library** `sub_0807f6a8` (four `sub_08021c14` terrain samples
  around a box whose six signed offsets come from `Actor.unk50`),
  `sub_08080374` (drain `Task.unk18` by N and fire cue 192 when it runs out),
  `sub_08081814` / `sub_08080b2c` / `sub_08082cc4` (the walk, flap and hop
  animation loops), `sub_08081e64` (flip `Task.unk43` from the
  `gUnk_03005550[4]` input, returning 1 when the input already matches the
  facing) and `sub_08082c5c` (turn around once every 30 frames if the player
  is behind and within 31 units).  `sub_0807fe18` is the module's only
  jump-table dispatch: a five-state machine over `Task.unk30` whose table
  sits at `0x0807FE3C`.
* **Cost** the bank is mechanical apart from `sub_0807f0c4`, which needed
  eleven separate source-shape decisions (lessons `3.206`-`3.209`) before it
  matched; 98 ROM tables were named through `split_config.json`
  `data_symbols`.

#### M21 census (pre-decompilation)
...
* **Census** the module really has **297** functions, not the 285 the sweep
  ...
  entries were missing - five that ARE pointer-referenced and were rejected
  only because `-fprologue-bugfix` left them without a `push`, so they read as
  the tail of the function above (`0x08086274`, `0x08087210`, `0x08089530`,
  `0x08089C0C`, `0x0808AA28`), and nine dead exports with their own
  `push {lr}` that nothing in the ROM points at (`0x08086128`, `0x080862FC`,
  `0x080868B8`, `0x08086A20`, `0x080870F4`, `0x08087298`, `0x08087824`,
  `0x0808798C`, `0x08088394`).  Two rows were false positives:
  `0x08087000`/`0x08087006` are arms of the `switch` in `sub_08086f54` (the
  9-entry jump table at `0x08086FA0` lands on `0x08087002`/`0x08087008`); the
  words that looked like pointers to them sit at `0x0827C564`/`0x0827C1AC`
  inside the compressed-graphics blob, the same artifact as `0x080671F8`.
* **What it turned out to be** fourteen scripted objects in the same
  three-table shape as M22/M24/M25/M26, plus two multi-state bosses and one
  shared terrain probe:
  * an **entry** installs the hooks (`Task.unk00` = `sub_080656b4` or
    `sub_080059d8`, `Task.unk0C` = `sub_0806523c` or `sub_08005d9c`,
    `Task.unk42` = 11/12), points `Task.unk38` at a TaskGfx block in
    `0x08752560-0x08752BD4` and hands `Task.unk73` to `sub_08002e98` with a
    per-script entry table: `0x08742064` (8), `0x08742570` (10),
    `0x087426B4` (4), `0x0874273C` (2), `0x08742798` (2), `0x087427B4` (2),
    `0x087428C0` (6) and `0x08742940` (2).
  * each row installs a per-frame **body** from a table indexed by
    `Task.unk14` and the body's resume function reads a **guard** table
    indexed by `Task.unk15`: `0x08742080`/`0x08742084`,
    `0x087420A0`/`0x087420A4`, `0x087420F0`/`0x08742100`,
    `0x08742104`/`0x08742108`, `0x0874212C`/`0x08742138` (3+3),
    `0x087425B0`/`0x087425B4`, `0x087425D0`/`0x087425D4`,
    `0x087425F8`/`0x087425FC`, `0x0874260C`/`0x08742610`,
    `0x0874263C`/`0x08742648` (3+3), `0x08742654`/`0x08742660` (3+3),
    `0x0874266C`/`0x08742684` (6+6), `0x087426AC`/`0x087426B0`,
    `0x087426C4`/`0x087426D8` (5+5), `0x08742704`/`0x08742710` (3+3),
    `0x0874271C`/`0x08742728` (3+3), `0x08742744`/`0x08742758` (5+5),
    `0x087427A0`/`0x087427A8` (2+2), `0x087428D8`/`0x087428E0` (2+2),
    `0x08742908`/`0x0874290C`, `0x08742910`/`0x08742918` (2+2) and
    `0x08742920`/`0x08742928` (2+2).
  * the **eleven-state boss** entered at `0x08089D20`
    (bodies `0x087427BC`, guards `0x087427E8`) walks its states with
    `sub_0808a270`/`sub_0808a36c`/`sub_0808a478`/`sub_0808a5b8`/`sub_0808a710`
    (each re-arms the entry when `Task.unk14` leaves its state, and every one
    of them ends by probing the tile under the actor through `sub_08021c14`).
    `sub_08089808` sets the sprite frame and the `Task.unk3E` H-flip bit from
    the 16-row `u8` pair table at `0x08742778`; `sub_0808a7f4` +
    `sub_0808a84c` roll a weighted move out of `0x08742854`/`0x08742856`/
    `0x0874285C` into `gUnk_030023B4`/`gUnk_030023D4`; `sub_0808a880` turns an
    `sub_08064314` heading into a `Task.unk2C` angle and a facing through the
    `u16` table at `0x08742862`.
  * the **twelve-state boss** entered at `0x0808BE3C`
    (bodies `0x08742948`, guards `0x08742978`) picks its next attack in
    `sub_0808c8bc` from the four weight tables the pointer array at
    `0x08742998` selects, aims a 16.16 jump in `sub_0808c980` with `Div`, and
    probes one tile to the left or right of `Task.unk48` with the
    `sub_0808ca00`/`sub_0808cab8` pair.  That pair is the module's only
    terrain code: it maps the point through `sub_08021b18` and then walks the
    same `0x100`-stride index chain M06 uses -
    `gUnk_087339F0` (must be non-zero) -> `0x087336F0` -> `0x087337F0` ->
    `gUnk_08732CF0` -> `0x08732FF0` (all must be zero) - before reading the
    per-tile height byte out of the row `0x087347F0[i]` (right) or
    `0x087343F0[i]` (left) at index `(x & 15) * 17`.
  * the shared **`sub_08086f54`** is a 9-way "which way can I walk" probe: it
    folds `Task.unk7A` and the room bytes `gUnk_03005550[0]`/`[1]`/`[4]` into
    a 0-32 index of the `s16` grid at `0x08742150` (16 columns of
    `Task.unk2C >> 5`) and writes the new `Task.unk2C` back when the entry is
    non-negative.
* **Size** 27.0 KiB (`0x6bf0`), 297 functions (266 reachable only through pointer tables), mean `0x60`, largest `0x1f8`, pool words 14.0% of bytes.
...
* **What it turned out to be** four self-contained boss scripts, one per
  ...
  * an **entry** function installs the draw hook (`Task.unk00` =
    `sub_080656b4`, `Task.unk0C` = one of `sub_08065438` / `sub_08065350` /
    `sub_080653ec` / `sub_0806523c`), points `Task.unk38` at the boss's
    `TaskGfx` block in `0x08752ED8-0x087536FC`, bumps the live-boss counter
    `gUnk_02007D00[0]`/`[7]`, and hands `Task.unk73` to `sub_08002e98`, which
    dispatches through the anchor table;
  * the **per-frame body** (`Task.unk04`) counts `Task.unk18`/`unk20` down,
    asks `sub_0806acf8`/`sub_080692fc` whether the player interrupted, and
    then dispatches `Task.unk15` through the same table; it also drives the
    three animation calls `sub_080639b4`/`sub_080639f0`/`sub_08063a00` off a
    per-state row (`0x087437D0`+`0x087437F4`... per family) and the
    graphics-reload pair `sub_080663f4`/`sub_08066468`;
  * each **state** is a `<body, guard>` pair: the body is a run of
    `TaskYieldTrampoline` waits that step `Task.unk3C` (the animation frame),
    clear and then wait on `Task.unk7A` (the flag the movement helpers set
    when the boss lands), and set the 16.16 velocity pair through
    `sub_080061c0` / `sub_0800622c` (`0x5A5A5A5A` is the "leave this axis
    alone" sentinel); the guard is the six-instruction
    `if (Task.unk14 != N) sub_08006148(body_dispatcher, gCurTaskIdx);` that
    re-arms the dispatcher whenever the state byte changes.
  Boss-specific pieces worth naming: `sub_080905d8` aims by dividing
  `|sub_08063cd0()|` by 3 (`Div`), `sub_0809074c` and `sub_080925b8` spawn the
  helper actors 8/30/110/133 through `sub_08064b5c`, `sub_08092250` picks the
  next attack by classifying `|sub_08063cd0()|` into `gUnk_02007D00[6]` and
  then running one of three `switch` arms, `sub_080934f8` probes the terrain
  with ten `sub_08021bb4` samples along `0x08743AB8` and turns the two result
  bits into a new `Task.unk28` direction via `0x08743AC2`, and
  `sub_08090fe0` / `sub_08091d24` / `sub_0809397c` / `sub_08093cf8` are the
  companion tasks that mirror a boss's position: each checks
  `gUnk_03004CA0[Task.unk44] != -1` (the parent is still alive), then copies
  16.16 position or animation state out of `gUnk_03002790[Task.unk44]`, and
  calls `sub_08005654(gCurTaskIdx)` to kill itself when the parent is gone.
* **Census fixes** two, both curated in `tools/symdb.py`: `0x08090904` is a
  false positive (the `bx r0` that ends `sub_080908ec`, pointed at by a
  coincidental byte-table word at `0x086C5964`), and `0x08093858` is a real
  four-instruction leaf the prologue scan could not propose because
  `-fprologue-bugfix` left it without a `push` - the behaviour-table word at
  `0x087440CC` points at it.  The module's true function count is 120.
* **ROM data** 77 new `tools/split_config.json` `data_symbols`: the per-boss
  state and script tables cluster in `0x08743734-0x08743B48`, the fourth
  boss's in `0x087440DC-0x08744324`, the `TaskGfx` blocks in
  `0x08752ED8-0x087536FC`, and three shared blobs sit outside those
  (`0x0826A668`, `0x0827565C`/`0x08275670`, `0x082959A8` - the 16-byte
  graphics records `sub_080663f4`/`sub_08066480` upload).

### M25 census (pre-decompilation)
...
* **What it turned out to be** four independent scripted enemies in the same
  ...
  standalone wanderer:
  * an **entry** (`sub_08093f64`, `sub_080941ac`, `sub_080957bc`,
    `sub_080974f8`) that installs `sub_080656b4` as the draw hook
    (`Task.unk00`) and `sub_08065438` or `sub_0806523c` as the per-frame hook
    (`Task.unk0C`), sets `Task.unk42` to 9 or 11, points `Task.unk38` at a
    `TaskGfx` block (`gUnk_087536FC`, `gUnk_08752F74`, `gUnk_08753718`,
    `gUnk_087537FC`), bumps the live-enemy counter `gUnk_02007D00[8]`, loads
    the animation script through `sub_080666cc` and hands `Task.unk73` to
    `sub_08002e98` with a one-word entry table (`0x087441CC`, `0x08744440`);
  * a **guard** table dispatched from `Task.unk14` and a **body** table
    dispatched from `Task.unk15`, the two tables consecutive in ROM:
    `0x087441D0`/`0x087441D4` (1+1), `0x08744444`/`0x08744470` (11+11),
    `0x087444E4`/`0x08744564` (12+13) and `0x0874489C`/`0x087448C0` (9+9);
  * one `<body, guard>` **pair per state**, the guard re-arming through
    `sub_08006148(sub_08094290 / sub_080975ac, gCurTaskIdx)`.
* **Shared machinery** `sub_080956e4` is the module's own `struct AnimCmd`
  walker (`-3` loops, `-2` stops and clears `Actor.unk2C`; odd frame ids go to
  `sub_08006364` and even ones to `sub_0800634c` when `Task.unk43 == 1`, and
  the other way round otherwise); `sub_08095794` and `sub_08095768` are its
  two step wrappers and `sub_080956c8` restarts a script; `sub_08094d10`
  classifies the four facings from `gUnk_03005550[4]` and `Task.unk43` and
  `sub_08094bbc` / `sub_08098748` turn its `s8` result into an animation swap;
  `sub_08094358` and `sub_08097024` are the animation-row selectors that map
  `Task.unk3C` ranges (4-9, 10-19, 20-27, 28-31, 32-35, 36-47) onto the
  `0x08744A04`-`0x08744CC0` descriptor rows.
* **The two companions** `sub_080985e0` (gfx `gUnk_08753054`) and
  `sub_0809876c` (gfx `gUnk_087538B0`) both use `Task.unk42` = 9 and
  `sub_0806523c` as the per-frame hook, and are `while (1)` loops that swap
  animation from `Task.unk2C` / `Task.unk34`.
...
* **Census fixes** eight curated `EXTRA_THUMB_ENTRIES` in `tools/symdb.py`, so
  ...
  table word at `0x087441D4`, `0x080963C0` from `0x08744588`, `0x08098738`
  from `0x087455E0`, `0x080988A4` from `0x087455E4`) and four are dead exports
  nothing in the ROM references (`0x08095768`, `0x08096E70`, `0x08098708`,
  `0x080988C0`). All eight were found by a reachability walk over the module's
  annotated listing (`docs/lessons-learned.md` 4.36).
* **ROM data** 131 new `tools/split_config.json` `data_symbols`: the state,
  animation and descriptor tables cluster in `0x0874433C-0x0874550C`, the
  `TaskGfx` blocks at `0x08752F74`, `0x08753054`, `0x087536FC`, `0x08753718`,
  `0x087537FC` and `0x087538B0`, and three graphics records outside those
  (`0x0826F170`, `0x082B07BC`, `0x082BFBA4`).

### M26 census (pre-decompilation)
...
### M34 `0x080B6154-0x080B9D0B` - save file / SRAM records + options - **landed (#94, partial: 97/105)**

* **Decompiled** into `src/save_b6290.c`, `save_b6a90.c`, `save_b6d04.c`, `save_b6e44.c`,
  `save_b77d4.c`, `save_b63a4.c`, `save_b6c40.c`, `save_b7a9c.c`,
  `save_b7df4.c`, `save_b6f38.c`, `save_b72bc.c`, `save_b79b8.c`,
  `save_b7e14.c`, `save_b8888.c`, `save_b8918.c`, `save_b8ea0.c` (97 of 105
  ...
  `0x0803DB34` (lesson 4.40, sixth instance); `sub_080b6b08` really runs
  `0x080B6B08-0x080B6C40`.
* **What it turned out to be** three subsystems, not one:
  * an **HBlank wavy-scroll effect** - `gUnk_0300003C` is the HBlank callback
    cell, `gUnk_02016490` its 0/1/2/3 state, `gUnk_02016494` a frame counter,
    and `gUnk_020164A0` the 480-halfword scroll table DMA'd to `REG_BG1HOFS` /
    `REG_BG2HOFS` (`gUnk_0300101C` holds the destination register address);
  * the **SRAM save file** - four 256-byte slots at `gUnk_0200E600`, mirrored
    to `0x0E000200` two 256-byte writes at a time.  Slot 3 is the scratch
    copy.  Each slot's `0x70` word is the additive 28-word checksum seeded
    `0x97538642` (`sub_080b7dd0`), an erased slot is 28 words of `0x99999999`,
    and `gUnk_030023E8` indexes the slot with the highest `unk08` generation
    counter (`sub_080b78e4`).  `sub_080b7af8` writes the `SRAM_V112` signature
    at `0x080CFE20` to `0x0E000000`;
  * the **link-play results screen** - a class-1 task whose coroutines occupy
    `0x080B9610-0x080B9D0C` and dispatch through the 14-entry anchor table
...
* **10 functions remain in asm**: `sub_080b6154`, `sub_080b6290`,
  `sub_080b6474`, `sub_080b6570`, `sub_080b67dc`, `sub_080b6b08`,
  `sub_080b6d04`, `sub_080b75a4`, `sub_080b76a8`, `sub_080b8694` (see #94 for
  the per-function residue).  Six of them are the four-loop HBlank family of 3.336 and its two
  `gUnk_03001E94` siblings, already down to 12-31 differing bytes of pure
  allocation.
